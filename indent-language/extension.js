...
function zorTaskTemplate() {
  return {
    version: '2.0.0',
    tasks: [
      {
        label: 'Build Current Zor File',
        type: 'shell',
        command: 'zor',
        args: ['build', '${file}'],
        group: 'build',
...
function activate(context) {
  ...
  function runInZorTerminal(command) {
    const terminalName = 'Zor';
    let terminal = vscode.window.terminals.find((t) => t.name === terminalName);
    if (!terminal) {
      terminal = vscode.window.createTerminal(terminalName);
    }
  ...
  const zorBuildCommand = vscode.commands.registerCommand('zor.buildCurrentFile', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showErrorMessage('Open a .zor file first.');
      return;
    }

    const filePath = editor.document.fileName;
    if (!filePath.toLowerCase().endsWith('.zor')) {
      vscode.window.showErrorMessage('The active file is not a Zor (.zor) file.');
      return;
    }
    ...
    const fileDir = path.dirname(filePath);
    runInZorTerminal(shellCommandInDir('zor build ' + shellQuote(filePath), fileDir));
  });

  const zorRunCommand = vscode.commands.registerCommand('zor.runCurrentFile', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showErrorMessage('Open a .zor file first.');
      return;
    ...
    const fileDir = path.dirname(filePath);
    runInZorTerminal(shellCommandInDir('zor run ' + shellQuote(filePath), fileDir));
  });
  ...
  context.subscriptions.push(zorBuildCommand);
  context.subscriptions.push(zorRunCommand);
